...
class QuestionVectorStore:
    def __init__(self, persist_directory: str = "backend/data/vectorstore"):
        """Initialize the vector store for Spanish listening comprehension questions"""
        ...
        self.collections = {
            "beginner": self.client.get_or_create_collection(
                name="beginner_questions",
                embedding_function=self.embedding_fn,
                metadata={"description": "Spanish listening comprehension questions - Beginner"}
            ),
            "intermediate": self.client.get_or_create_collection(
                name="intermediate_questions",
                embedding_function=self.embedding_fn,
                metadata={"description": "Spanish listening comprehension questions - Intermediate"}
            ),
            "advanced": self.client.get_or_create_collection(
                name="advanced_questions",
                embedding_function=self.embedding_fn,
                metadata={"description": "Spanish listening comprehension questions - Advanced"}
            )
        }

    def add_questions(self, level: str, questions: List[Dict], video_id: str):
        """Add questions to the vector store"""
        if level not in ["beginner", "intermediate", "advanced"]:
            raise ValueError("2 Only beginner, intermediate, and advanced sections are currently supported")
        ...
        for idx, question in enumerate(questions):
            ...
            question_id = f"{video_id}_{level}_{idx}"
    ...
    def search_similar_questions(self,level: str, query: str, 
        n_results: int = 5
    ) -> List[Dict]:
        """Search for similar questions in the vector store"""
        if level not in ["beginner", "intermediate", "advanced"]:
            raise ValueError("3 Only beginner, intermediate, and advanced sections are currently supported")
    ...
    def get_question_by_id(self, level: str, question_id: str) -> Optional[Dict]:
        """Retrieve a specific question by its ID"""
        if level not in ["beginner", "intermediate", "advanced"]:
            raise ValueError("1 Only beginner, intermediate, and advanced sections are currently supported")
    ...
    def index_questions_file(self, filename: str, level: str):
        ...
        # Add to vector store
        if questions:
            self.add_questions(level, questions, video_id)
...
if __name__ == "__main__":
    ...
    question_files = [
        ("backend/data/questions/sample1.txt", "beginner"),
        ("backend/data/questions/sample2.txt", "intermediate"),
        ("backend/data/questions/sample3.txt", "advanced")
    ]
    
    for filename, level in question_files:
        if os.path.exists(filename):
            store.index_questions_file(filename, level)
    
    # Search for similar questions
    similar = store.search_similar_questions("beginner", "Una pregunta sobre el cumpleaños", n_results=1)
    print(similar)